...
def process_pdf_folder(input_dir: str, output_dir: str = None):
    """
    Process all PDFs in a folder and save as JSON
    
    Args:
        input_dir: Directory containing PDF files
        output_dir: Directory to save JSON files (defaults to input_dir/extracted)
    """
    ...
    for pdf_file in pdf_files:
        try:
            output_file = save_to_json(str(pdf_file), str(output_path))
            print(f"✓ {pdf_file.name} → {Path(output_file).name}")
        except Exception as e:
            print(f"✗ {pdf_file.name} - Error: {e}")
    
    print("=" * 80)
    print(f"Completed! Files saved to: {output_path}")
...
#python ken_text_extraction.py ../../../data/2018_Recombinant_Spidroins.pdf